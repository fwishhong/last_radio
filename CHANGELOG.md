...
### Changed
- Save schema bumped to v5 (post-v0.5.0); v4 saves auto-migrate
  via `migrate_v4_to_v5(doc)` which adds `tutorial_done_step4`
  default false.

### Notes
- Merge: PR #2 (`9e3bfec`), `--squash` of `5fef441` + `bd4d512`
  + `f0b88db` per team release flow. Tag forthcoming:
  `v0.6-m13-done`.
- Post-merge verification: canonical 22-suite headless gate all
  green (`actor_regression_test` pre-existing failure on master
  unrelated; see `release_roadmap.md` known-issues).
